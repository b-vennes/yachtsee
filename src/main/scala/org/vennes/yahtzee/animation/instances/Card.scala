...
given showCardWithOptions: Show[(Card, DiceSet, Option[Card.Opt])] = values =>
  val card = values._1
  val dice = values._2
  val choosing = values._3
...
val animateCardWithOptions: Animation[(Card, DiceSet, Option[Card.Opt])] =
  Animation.fromShow()
